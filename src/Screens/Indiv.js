...
import { FiArrowLeft, FiSearch, FiSunset } from 'react-icons/fi';
...
const PaintingServices = () => {
  ...
  return (
    <div className={`${isDarkMode ? 'bg-gray-900' : 'bg-white'} min-h-screen`}>
      ...
      <div className="flex items-center justify-between p-4 sticky top-0 z-50 bg-inherit">
        <button onClick={handleBack} className="p-2">
          <IoIosArrowBack size={24} color={isDarkMode ? '#fff' : '#000'} />
        </button>
        <h1 className="text-xl font-bold text-center flex-1 mx-4 text-gray-800">
          {t(`service_${sid}`) || name}
        </h1>
      ...
      <div className={`flex items-center rounded-lg mx-4 my-4 ${isDarkMode ? 'bg-gray-700' : 'bg-yellow-50'}`}>
        <div className="flex-1 p-4">
          <div className="mb-2">
            ...
            <p className="text-base mt-1 font-semibold text-gray-600">
              {t(`service_${sid}`) || name}
            </p>
            <p className="text-sm mt-1 opacity-80 text-gray-500">
...
const ServiceItem = React.memo(
  ({ title, imageUrl, handleBookCommander, serviceId, isDarkMode, t, id }) => {
    ...
    const isTablet = width >= 600;
    return (
      <div className={`flex flex-row items-center gap-4 p-4 rounded-lg ${isDarkMode ? 'bg-gray-800' : 'bg-white'} shadow mb-4`}>
        <div>
          <img
